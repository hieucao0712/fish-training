...
import { DragonCacheData, BoneCacheValue } from './GodCacheData2024';
...
import { SetZIndex, isPointInScreen } from '../../../../cc-common/cc30-fishbase/Scripts/Utilities/gfUtilities';
...
const dustColor = [
    { start: color(100, 200, 255, 255), end: color(0, 150, 255, 255) },
    { start: color(255, 245, 125, 175), end: color(255, 185, 0, 255) },
    { start: color(255, 200, 125, 190), end: color(255, 165, 0, 255) },
    { start: color(255, 180, 125, 200), end: color(255, 90, 0, 255) },
    { start: color(255, 150, 125, 175), end: color(255, 25, 0, 255) },
];
...
export class Dragon2024 extends gfDragon {
    ...
    _timeRemain: any = null;
    _isWaitingToMove: boolean = false;
    _isLoaded: boolean = false;
    _state: any = 1;
    _oldState: any = 1;
    _colorTween: any = null;
    _plasmaTween: any = null;
    _duration: any = 0;
    _mainMaterial: any;
    _isOutScreen: any;
    curAnimTime: any;
    ...
    onLoad(): void {
        ...
        this.listBox = this.getComponents(BoxCollider2D);
        SetZIndex(this.node, 900);
    ...
    setupBones() {
        const obj = this.fishAnim.skeletonData[animationName].paths;
    ...
    updateParticlePos() {
        for (let i = 0; i < 3; ++i) {
            let bone = this.fishAnim.findBone(boneParticle[i]);
            {
                ...
                if (particle) {
                    particle.setPosition(bone.worldX, bone.worldY)
    ...
    initFishData(data) {
        this.node.setPosition(v3(GameConfig.instance.AppSize.Width / 2, GameConfig.instance.AppSize.Height / 2));
        ...
        this._mainMaterial = this.fishAnim.material;
        this._mainMaterial.setProperty('brightness', 0.0);
        this.unscheduleAllCallbacks();
        stopAllActions(this.fishAnim.node);
        this._colorTween && this._colorTween.stop();
        this._plasmaTween && this._plasmaTween.stop();
        ...
        this.startMoving();
        this.schedule(this.checkOutScreen, 0.2, macro.REPEAT_FOREVER, 0.1);
    ...
    onHitState(data) {
        ...
        switch (TypeWin) {
            ...
            case 3: //Plasma
                ...
                let callback = () => {
                    const bodyPos = this.convertToWorldSpaceAR(this.node, this.listBox[this.listBox.length / 5 | 0].offset);
                    const dataReward = {
                        ListFish,
                        WinAmount,
                        DeskStation,
    ...
    changeColor() {
        ...
        if (!this._colorTween || this._colorTween.isDone()) {
            this._colorTween = tween(this.fishAnim)
                .call(() => {
    ...
    playPlasmaEffect(callback) {
        ...
        if (!this._plasmaTween || this._plasmaTween.isDone()) {
            ...
            this._plasmaTween = tween(this.fishAnim)
                .call(() => {
                    if (isRed)
    ...
    playDropCrystal(data) {
        ...
        const worldPos = this.convertToWorldSpaceAR(this.node,this.listBox[0].offset);
    ...
    playEffectDie() {
        ...
        tween(this.fishAnim.node)
            ...
            .by(0.05, { position : v3(0, 10) })
            .call(() => {
                this._mainMaterial.setProperty('brightness', 0.3);
            })
            .by(0.065, { position : v3(0, -10) })
            .call(() => {
                this._mainMaterial.setProperty('brightness', 0.45);
            })
            .by(0.065, { position : v3(0, -10) })
            ...
            .by(0.065, { position : v3(10, 10)})
            .call(() => {
                this._mainMaterial.setProperty('brightness', 0.8);
            })
            .by(0.065, { position : v3(-10, 0)})
            .call(() => {
                ...
                this.fishAnim.useTint = true;
                this.fishAnim.setToSetupPose();
                sys.isNative ? this.fishAnim._updateRealtime(curAnimTime) : this.fishAnim.update(curAnimTime);
            })
            .by(0.065, { position : v3(-10, 0)})
            .call(() => {
                this._mainMaterial.setProperty('brightness', 0.8);
            ...
            .call(() => {
                ...
                Emitter.instance.emit(EventCode.EFFECT_LAYER.SHOW_POPUP_WIN_JACKPOT);
            })
            .by(0.065, { position : v3(0, -10)})
            .by(0.065, { position : v3(10, 10)})
            .call(() => {
                ...
                for (let i = 0; i < explBones.length; ++i) {
                    explPositions.push(this.convertToWorldSpaceAR(this.node,v2(this.bone[explBones[i]].worldX, this.bone[explBones[i]].worldY)));
                }
    ...
    //     sys.isNative ? this.fishAnim._updateRealtime(timePassed) : this.fishAnim.update(timePassed);
    ...
    startMoving() {
        Emitter.instance.emit(EventCode.SOUND.PLAY_SOUND_BACKGROUND, GameConfig.instance.SOUND_BACKGROUND_CONFIG_2024.GODZILLA);
    ...
    checkOutScreen(): boolean {
        ...
        const head = this.node.parent.getComponent(UITransform).convertToNodeSpaceAR(this.node.getComponent(UITransform).convertToWorldSpaceAR(v3(this.listBox[0].offset.x, this.listBox[0].offset.y, 0)));
        const foot = this.node.parent.getComponent(UITransform).convertToNodeSpaceAR(this.node.getComponent(UITransform).convertToWorldSpaceAR(v3(this.listBox[this.listBox.length - 1].offset.x,this.listBox[this.listBox.length - 1].offset.y, 0)));
        const tail = this.node.parent.getComponent(UITransform).convertToNodeSpaceAR(this.node.getComponent(UITransform).convertToWorldSpaceAR(v3(this.listBox[this.listBox.length - 2].offset.x,this.listBox[this.listBox.length - 2].offset.y, 0)));
        ...
        if (lastState && !this._isOutScreen) {
            ...
            this.schedule(this.updateParticlePos, 0.2, macro.REPEAT_FOREVER, 0);
            Emitter.instance.emit(EventCode.FISH_LAYER.BOSS_ON_GAME);
        }
    }

    getLockPositionByNodeSpace(node) {
        const head = this.convertToWorldSpaceAR(this.node,this.listBox[3].offset);
        const tail = this.convertToWorldSpaceAR(this.node,this.listBox[this.listBox.length - 2].offset);
        const foot = this.convertToWorldSpaceAR(this.node,this.listBox[this.listBox.length - 1].offset);
    ...
    convertToWorldSpaceAR(node: Node, pos){
        return node.getComponent(UITransform).convertToWorldSpaceAR(v3(pos.x, pos.y, 0));
    }

    convertToNodeSpaceAR(node: Node, pos){
        return node.getComponent(UITransform).convertToNodeSpaceAR(v3(pos.x, pos.y, 0));
    }
